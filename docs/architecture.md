...
- `/services`: Contém os microsserviços executáveis. Cada serviço é 100% independente para otimizar o contexto de build do Docker.
...
## Padrões de Conteinerização e DevOps
O projeto segue uma abordagem "DevOps-first", focada em deploy contínuo via GitOps (GitHub Actions -> ArgoCD -> Kubernetes).

Para garantir segurança, performance e builds otimizados, todos os microsserviços em `/services` devem conter seu próprio `Dockerfile` e `.dockerignore` seguindo estas diretrizes rigorosas:
- **Build Context Isolado:** Não utilizamos pastas compartilhadas (como um antigo diretório `/libs`). O contexto de build do Docker é sempre a raiz do próprio serviço.
- **Multi-stage Build:** Separação clara entre o estágio de build (JDK) e o estágio de runtime (JRE Alpine enxuta) para evitar o vazamento de código-fonte e ferramentas de compilação.
- **Cache de Camadas:** Os arquivos do Maven (`.mvn`, `mvnw`, `pom.xml`) devem ser copiados e as dependências baixadas em modo offline *antes* de copiar o código-fonte (`src/`). Isso maximiza o uso do cache do Docker no CI.
- **Segurança (Non-root):** Nenhum container roda como usuário `root`. A imagem final de produção utiliza um grupo e usuário genéricos com IDs numéricos (UID `1001`, GID `1001`) para compatibilidade nativa com políticas restritivas do Kubernetes (SecurityContext).
...
2. **Estrutura de Pastas:** Criar as pastas `/services` e `/platform` na raiz do projeto.
